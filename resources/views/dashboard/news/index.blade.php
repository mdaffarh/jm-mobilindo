    ...
    <div class="p-4 pt-8 sm:ml-64">
        <div class="p-4 border-2 border-dark-orange border-dashed rounded-lg mt-14 font-sans ">
            <section class="mt-4 mb-6">
                <span class="text-4xl font-bold font-clash text-dark-orange">Kelola Berita</span>
            </section>
            <section>
                <div class="mx-auto">
                    <div class="relative overflow-hidden bg-white shadow-md sm:rounded-lg border border-orange-500">
                        ...
                        <div class="overflow-x-auto pb-18">
                            <table class="w-full text-sm text-left text-gray-500 ">
                                {{-- id, title, content, thumbnail_path (nullable), status, --}}
                                    ...
                                    @foreach ($news as $n)
                                        <tr class="border-b  hover:bg-orange-100">
                                            ...
                                            <th scope="row"
                                                ...
                                                @else
                                                    <span
                                                        class="bg-gray-100 text-gray-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm capitalize">
                                                        {{ $n->status }}
                                                    </span>
                                                @endif
                                            </th>
                                            <td class="px-4 py-3 flex items-center">
                                                <button id="dropdown{{ $n->id }}-button"
                                                    data-dropdown-toggle="dropdown{{ $n->id }}"
                                                ...
                                                <div id="dropdown{{ $n->id }}"
                                                    class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow ">
                                                    <ul class="py-1 text-sm text-gray-700 "
                                                        aria-labelledby="dropdown{{ $n->id }}-button">
                                                        <li>
                                                            <a href="{{ route('news.show', $n->id) }}"
                                                                class="block py-2 px-4 hover:bg-gray-100 w-full text-left cursor-pointer">
                                                                Preview
                                                            </a>
                                                        </li>
                                                        <li>
                                                        ...
                                                        @if ($n->status != 'published')
                                                            <li>
                                                                <a href="#"
                                                                    onclick="statusConfirm(event, 'form-publish-{{ $n->id }}', 'publish')"
                                                                    class="block py-2 px-4 hover:bg-gray-100 text-green-700">Publish</a>
                                                            </li>
                                                        @endif
                                                        @if ($n->status != 'archived')
                                                            <li>
                                                                <a href="#"
                                                                    onclick="statusConfirm(event, 'form-archive-{{ $n->id }}', 'arsipkan')"
                                                                    class="block py-2 px-4 hover:bg-gray-100 text-red-700">Arsipkan</a>
                                                            </li>
                                                        @endif

                                                        <!-- Form Publish -->
                                                        <form id="form-publish-{{ $n->id }}"
                                                            action="{{ route('news.updateStatus', ['news' => $n->id, 'status' => 'published']) }}"
                                                        ...
                                                        <!-- Form Pembatalan -->
                                                        <form id="form-archive-{{ $n->id }}"
                                                            action="{{ route('news.updateStatus', ['news' => $n->id, 'status' => 'archived']) }}"
                                                            method="POST" style="display: none;">
                                                            @csrf
                                                            @method('PATCH')
                                                        </form>
                                                    ...
                                                    <div class="py-1">
                                                        ...
                                                        <form id="delete-form-{{ $n->id }}"
                                                            action="{{ route('news.destroy', $n->id) }}" method="POST"
                                                            style="display: none;">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                        <a href="#"
                                                            onclick="deleteConfirm(event, 'delete-form-{{ $n->id }}')"
                                        ...
                                        {{-- Modal Show Data --}}
                                        <div id="modal-show-{{ $n->id }}" tabindex="-1" aria-hidden="true"
                                                ...
                                                <div class="relative bg-white rounded-lg shadow">
                                                    ...
                                                    <div
                                                        ...
                                                        <h3 class="text-xl font-semibold text-gray-900">
                                                            Detail Berita: {{ $n->title }}
                                                        </h3>
                                                        <button type="button"
                                                            data-modal-hide="modal-show-{{ $n->id }}"
                                                    ...
                                                    <div class="p-4 md:p-5 space-y-4">
                                                        <div class="text-center">
                                                            @if ($n->thumbnail_path)
                                                                <img src="{{ asset('storage/' . $n->thumbnail_path) }}"
                                                                    alt="Thumbnail {{ $n->title }}"
                                                                    class="mx-auto w-44 h-44 object-contain rounded-lg border border-gray-200 mb-4">
                                                            @endif
                                                            <p class="text-lg text-gray-900">
                                                                <strong>Judul:</strong> {{ $n->title }}
                                                            </p>
                                                            <p class="text-gray-700 whitespace-pre-line">
                                                                <strong>Konten:</strong><br> {!! $n->content !!}
                                                            </p>
                                                            <p class="text-gray-900">
                                                                <strong>Status:</strong> {{ ucfirst($n->status) }}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div
                                                        ...
                                                        <button data-modal-hide="modal-show-{{ $n->id }}"
                                                            type="button"
...
@push('scripts')
    <script>
        ...
        function statusConfirm(event, formId, actionText) {
            event.preventDefault();
            Swal.fire({
                text: `Anda akan melakukan ${actionText} berita ini.`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d33',
                cancelButtonColor: '#aaa',
